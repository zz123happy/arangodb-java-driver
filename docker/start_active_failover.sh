...
# EXAMPLE:
#   ./start_active_failover.sh docker.io/arangodb/arangodb:3.6.0
...
docker run -d -v "$LOCATION"/jwtSecret:/jwtSecret -e ARANGO_LICENSE_KEY="$ARANGO_LICENSE_KEY" --network arangodb --ip 172.28.3.1 --name server1 "$1" arangodb --starter.mode=activefailover --starter.join server1,server2,server3 --auth.jwt-secret /jwtSecret
docker run -d -v "$LOCATION"/jwtSecret:/jwtSecret -e ARANGO_LICENSE_KEY="$ARANGO_LICENSE_KEY" --network arangodb --ip 172.28.3.2 --name server2 "$1" arangodb --starter.mode=activefailover --starter.join server1,server2,server3 --auth.jwt-secret /jwtSecret
docker run -d -v "$LOCATION"/jwtSecret:/jwtSecret -e ARANGO_LICENSE_KEY="$ARANGO_LICENSE_KEY" --network arangodb --ip 172.28.3.3 --name server3 "$1" arangodb --starter.mode=activefailover --starter.join server1,server2,server3 --auth.jwt-secret /jwtSecret
